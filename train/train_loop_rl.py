...
def train_loop(
    stream,
    bocpd_params: Dict[str, List[Any]]=None,
    vae_params: Dict[str, List[Any]]=None,
    rl_params: Dict[str, List[Any]]=None,
    joint_params: Dict[str, List[Any]]=None,
    num_epochs=50,
    save_dir="checkpoints",
    total_steps=10000,
    device='cpu'
):
    state_window=tuning_params['state_window']
    ...
    actor = Actor(state_dim=state_window, z_dim=rl_params['state_dim'], hidden_dim=rl_params['hidden_dim'], action_dim=rl_params['action_dim']).to(device)
    critic = Critic(state_dim=state_window, z_dim=rl_params['state_dim'], hidden_dim=rl_params['hidden_dim']).to(device)
    encoder = VAEEncoder(input_dim=vae_params['input_dim'], hidden_dim=vae_params['hidden_dim'], z_dim=vae_params['latent_dim'], seq_len=seq_len_for_vae).to(device)
